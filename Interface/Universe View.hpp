...
Makes sure that universeX will not result in a left side out of bounds error.
*/
void checkUniverseLeft() { if (universeX < 0) universeX = 0; }
...
Makes sure that universeY will not result in a upper side out of bounds error.
*/
void checkUniverseUp() { if (universeY < 0) universeY = 0; }
...
inline void renderShips(int x, int y, GalaxyTile* tile) {
	...
	if (!tile->numFleets) return;

	// Renders all Ships in the GalaxyTile.
	// TODO implement

	// TODO DEBUG REMOVE
	int numShips = tile->fleets[0]->ships[0].num;
	numShips = numShips > 255 ? 255 : numShips;

	// TODO DEBUG REMOVE
	SDL_Rect shipSprite = getShipSprite(tile->fleets[0]->ships[0].type.sprite);
...
void renderUniverse() {
	...
	// Renders each tile in the galaxy.
	for (int i = 0; i < universeViewport.w; i += universeTileSize) {
		for (int j = 0; j < universeViewport.h; j += universeTileSize) {
			...
			sprite = uIndex(i / universeTileSize + universeX, j / universeTileSize + universeY).getSprite();
			SDL_RenderCopy(renderer, tileSheet, &sprite, &tile);

			// Renders the tile's owner.
			if (uOwner(i / universeTileSize + universeX, j / universeTileSize + universeY)) {
				SDL_Colour governmentColour = uOwner(i / universeTileSize + universeX, j / universeTileSize + universeY)->flagColour;
			...
			if (uIndex(i / universeTileSize + universeX, j / universeTileSize + universeY).getTileID() == SYSTEM_TILE) {
				renderUniverseSystem(tile, i / universeTileSize + universeX, j / universeTileSize + universeY);

			}

			// Renders the tile's Ships.
			renderShips(i, j, &uIndex(i / universeTileSize + universeX, j / universeTileSize + universeY));
